...
Actually check working directory before downloading and unzipping data.
```{r}
getwd()
```
...
```{r}
weightmedian(directory = "hw/diet_data", day = 20)
weightmedian("hw/diet_data", 4)
weightmedian("hw/diet_data", 17)
```
...
```{r}
temp <- lapply(files_full, read.csv)
str(temp)
```
