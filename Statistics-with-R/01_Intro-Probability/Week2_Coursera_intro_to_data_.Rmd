...
# Delta and United Airlines have highest IQR
```
...
#Ans: LGA has the best on-time departure rate
```
...
#N666DN is fastest at 703.4 mph
```
...
nycflights %>% 
  group_by(dep_type) %>% 
  summarise(ot_arr_rate_by_dep = sum(arr_type == "on time") / n()) %>% 
  arrange(desc(ot_arr_rate_by_dep))

#Ans: 0.183 (18.3%) of flights delayed in departing ended up arriving ontime
```
